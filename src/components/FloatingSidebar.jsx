...
const IconWrapper = ({ children, onClick, href }) => {
  const content = (
    <div 
      className="w-14 h-14 mx-2 md:mx-0 md:my-2 flex items-center justify-center rounded-full cursor-pointer transition-all duration-300 hover:scale-110"
      style={{
...
export default function FloatingSidebar() {
  const handleDownload = () => {
    ...
    link.href = '/sample-brochure.pdf';
    link.download = 'TeamWorks-Brochure.pdf';
  ...
  return (
    <>
      {/* Mobile Layout - Horizontal at bottom */}
      <div className="fixed bottom-5 left-1/2 transform -translate-x-1/2 z-50 md:hidden">
        <div 
          className="flex flex-row items-center p-2 rounded-full shadow-2xl border border-white/30"
          style={{
            background: 'rgba(255, 255, 255, 0.2)',
            backdropFilter: 'blur(12px)',
          }}
        >
          <IconWrapper href="/about-us">
            {/* Icon 1: Document with pencil - About Us */}
          ...
          <IconWrapper onClick={handleDownload}>
            {/* Icon 2: Document with download - PDF Download */}
            <svg className="w-7 h-7 text-white" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <IconWrapper href="/contact-us">
            {/* Icon 3: Phone - Contact Us */}
            <svg className="w-7 h-7 text-white" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* Desktop Layout - Vertical on right side */}
      <div className="hidden md:block fixed right-5 top-1/2 transform -translate-y-1/2 z-50">
        <div 
